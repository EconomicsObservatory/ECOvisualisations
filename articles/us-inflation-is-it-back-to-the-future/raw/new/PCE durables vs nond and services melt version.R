...
library(reshape2)
pce_data <- read.csv(file = "pceinflationUSE1.csv", fileEncoding = "UTF-8-BOM")
str(pce_data)
...
ggplot(meltdf1, aes(Date , y=value , color=variable )) + geom_line()+
  ...
  xlim(as.Date(c("2015-01-01" , NA))) +
  ggtitle(" ") + labs(caption = "Source: BLS, FRED") + 
...
#THIS IS CHART 4
# produces Chart of real Consumption Indexed on December 2019  
#melts df into one column using date as id
...
ggplot(meltdf2, aes(Date , y=value , color=variable )) + geom_line()+
  ...
  ggtitle("") + labs(caption = "Source: BEA, FRED") + 
